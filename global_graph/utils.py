...
def sparse_mx_to_torch_sparse_tensor(sparse_mx) \
    ...
    sparse_mx = sparse_mx.tocoo().astype(np.float32) #将输入的稀疏矩阵 sparse_mx 转换为 COO 格式（coordinate format），并将数据类型转换为 np.float32
    indices = torch.from_numpy(
        np.vstack((sparse_mx.row, sparse_mx.col)).astype(np.int64) #使用 np.vstack 将稀疏矩阵的行索引和列索引堆叠在一起，获得稀疏矩阵中非零元素的索引
    )
    values = torch.from_numpy(sparse_mx.data)#将稀疏矩阵的非零元素值提取出来，并转换为 PyTorch 张量
    shape = torch.Size(sparse_mx.shape) #表示稀疏矩阵的形状
    return torch.sparse.FloatTensor(indices, values, shape)#与shape的形状保持一致 都是（544，544）
#将 scipy 的稀疏矩阵对象转换为 PyTorch 中的稀疏张量的功能，以便在深度学习模型中处理图数据时使用
